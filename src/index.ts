  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    try {
      ...
      switch (method) {
        case 'GET':
        case 'HEAD':
          return handle_get_head(request, bucket, method === 'HEAD');
        case 'PUT':
          return handle_put(request, bucket);
        case 'DELETE':
          return handle_delete(request, bucket);
        case 'MKCOL':
          return handle_mkcol(request, bucket);
        case 'COPY':
          return handle_copy(request, bucket);
        case 'MOVE':
          return handle_move(request, bucket);
        default:
